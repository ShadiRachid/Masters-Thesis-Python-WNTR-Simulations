...
for T in Tariff.columns:
    ...
    max_demand = total_hourly_demand(wn, water_tariff[T])[
        'Total Demands'].max()
    ...
    for e in E:
        ...
        for demand in new_demands_df.columns:
            QT = 0
            response_details.at[demand,
                                'Demand Before'] = original_demands_df[demand].sum()
            for hour in new_demands_df.index:
                if water_tariff.loc[hour, T] > water_tariff[T].mean():
                    ...
                    QT += abs(original_demands_df.loc[hour,
                              demand] - new_demands_df.loc[hour, demand])
            response_details.at[demand, 'shifted demand'] = round(QT, 3)
            response_details.at[demand, '% of demand shifted'] = round(
                (QT*100)/original_demands_df[demand].sum(), 2)

            for hour in new_demands_df.index:
                if water_tariff.loc[hour, T] < water_tariff[T].mean():
                    Delta = (
                        QT/RT)*abs(water_tariff[T].mean() - water_tariff.loc[hour, T])
                    new_demands_df.at[hour,
                                      demand] = original_demands_df.loc[hour, demand] + Delta
            response_details.at[demand,
                                'Demand After'] = new_demands_df[demand].sum()
        ...
        if total_hourly_demand(wn, water_tariff[T])['Total Demands'].max() <= ((1+M)*max_demand):
            ...
            ###
            energy_savings_percentage.at[e, T] = round(
                (new_energy - original_energy)*100/original_energy, 2)
            cost_savings_percentage.at[e, T] = round(
                (original_cost - new_cost)*100/original_cost, 2)
            revenue_increase_percentage.at[e, T] = round(
                (new_revenue - original_revenue)*100/original_revenue, 2)
            net_increase_percentage.at[e, T] = round(
                (new_net - original_net)*100/abs(original_net), 2)
            total_demand_difference.at[e, T] = round(
                response_details['Demand Before'].sum() - response_details['Demand After'].sum(), 4)
            Demand_shifted_percentage.at[e, T] = round(response_details['shifted demand'].sum(
            ) * 100 / response_details['Demand Before'].sum(), 2)

water_tariff.to_excel(
    r'3 Our Tariff Results/0.Water Tariffs.xlsx', header=True)
energy_savings_percentage.to_excel(
    r'3 Our Tariff Results/1.Energy Savings Percentage.xlsx', header=True)
cost_savings_percentage.to_excel(
    r'3 Our Tariff Results/2.Cost Savings Percentage.xlsx', header=True)
revenue_increase_percentage.to_excel(
    r'3 Our Tariff Results/3.Revenue Increase Percentage.xlsx', header=True)
net_increase_percentage.to_excel(
    r'3 Our Tariff Results/4.Net Increase Percentage.xlsx', header=True)
total_demand_difference.to_excel(
    r'3 Our Tariff Results/5.Total Demand Difference.xlsx', header=True)
Demand_shifted_percentage.to_excel(
    r'3 Our Tariff Results/6.Demand Shifted Percentage.xlsx', header=True)
...
writer1 = pd.ExcelWriter(
    r'3 Our Tariff Results/A. Demands.xlsx', engine='xlsxwriter')
...
hourly_results['Original Demand'] = total_hourly_demand_original['Total Demands'].round(
    3)
...
hourly_results['Original Net'] = hourly_results['Original Revenue'] - \
    hourly_results['Original Energy Cost']
...
for demand in new_demands_df.columns:
    ...
    for hour in new_demands_df.index:
        if water_tariff.loc[hour, T] < water_tariff[T].mean():
            Delta = (QT/RT) * \
                abs(water_tariff[T].mean() - water_tariff.loc[hour, T])
            new_demands_df.at[hour,
                              demand] = original_demands_df.loc[hour, demand] + Delta
...
summary.at['value', 'energy_savings_percentage'] = round(
    (new_energy - original_energy)*100/original_energy, 2)
summary.at['value', 'cost_savings_percentage'] = round(
    (original_cost - new_cost)*100/original_cost, 2)
summary.at['value', 'revenue_increase_percentage'] = round(
    (new_revenue - original_revenue)*100/original_revenue, 2)
summary.at['value', 'net_increase_percentage'] = round(
    (new_net - original_net)*100/abs(original_net), 2)
summary.at['value', 'total_demand_difference'] = round(
    response_details['Demand Before'].sum() - response_details['Demand After'].sum(), 1)
summary.at['value', 'Demand_shifted_percentage'] = round(
    response_details['shifted demand'].sum() * 100 / response_details['Demand Before'].sum(), 2)

### Exporting to Hourly Results Dataframe ###
hourly_results['New Demand'] = total_hourly_demand_new['Total Demands'].round(
    3)
hourly_results['Shifted Demand'] = hourly_results['New Demand'] - \
    hourly_results['Original Demand']
hourly_results['% Shifted Demand'] = round(
    hourly_results['Shifted Demand'] * 100 / hourly_results['Original Demand'], 2)
hourly_results['New Energy Consumption (KWh)'] = pump_energy['Energy (kWh)']
hourly_results['Energy Difference'] = round(
    hourly_results['Original Energy Consumption (KWh)'] - hourly_results['New Energy Consumption (KWh)'], 2)
hourly_results['% Energy Difference'] = round(
    hourly_results['Energy Difference']*100/hourly_results['Original Energy Consumption (KWh)'].sum(), 2)
hourly_results['New Energy Cost'] = pump_energy['Cost']
hourly_results['Cost Difference'] = hourly_results['Original Energy Cost'] - \
    hourly_results['New Energy Cost']
hourly_results['% Cost Differnce'] = round(
    hourly_results['Cost Difference']*100/hourly_results['Original Energy Cost'].sum(), 2)
hourly_results['New Revenue'] = total_hourly_demand_new["Revenue ($)"]
hourly_results['New Net'] = hourly_results['New Revenue'] - \
    hourly_results['New Energy Cost']
hourly_results['Net Difference'] = hourly_results['New Net'] - \
    hourly_results['Original Net']
hourly_results['% Net Difference'] = hourly_results['Net Difference'] * \
    100/abs(hourly_results['Original Net'])

writer1.save()
response_details.to_excel(
    r'3 Our Tariff Results/B. Response_Details.xlsx', header=True)
summary.to_excel(r'3 Our Tariff Results/C. Summary.xlsx', header=True)
hourly_results.to_excel(
    r'3 Our Tariff Results/D. Hourly Results.xlsx', header=True)
...
df = pd.read_excel(
    '3 Our Tariff Results/2.Cost Savings Percentage.xlsx', index_col=0)
...
Y = df2['y'].tolist()
Z = df2['z'].tolist()
fig.suptitle('Cost Savings Percentages across Tariff Scenarios and Elasticities',
...
ax.set_title('Cost Savings Percentages across Elasticities (Tariff Scenarios: ' +
             T1+' ,'+T2+' and '+T3+')', fontdict={'fontsize': 30})
...
for i in df3.columns:
    ax.plot(df3.index.tolist(), df3[i].tolist(),
            c[j], label=i+' Energy Tariff')
    j += 1
...
fig, ax = plt.subplots(figsize=(25, 10))
ax.set_title('Cost Savings Percentages across Elasticities (Random Tariff Scenarios)',
...
ax.plot(df4.index.tolist(), df4.mean(axis=1).tolist(),
        color='r', linestyle='dashdot', linewidth=3, label='μ')
ax.plot(df4.index.tolist(), df4.mean(axis=1).tolist()+df4.std(axis=1),
        color='r', linestyle='dotted', linewidth=3, label='μ+σ')
ax.plot(df4.index.tolist(), df4.mean(axis=1).tolist()-df4.std(axis=1),
        color='r', linestyle='dotted', linewidth=3, label='μ-σ')
ax.legend(fontsize=20)
...
###
df = pd.read_excel(
    '3 Our Tariff Results/3.Revenue Increase Percentage.xlsx', index_col=0)
df.replace(np.nan, 0, inplace=True)
df.columns = plotting_columns
df2 = pd.DataFrame(columns=['x', 'y', 'z'])
j = 0
...
Y = df2['y'].tolist()
Z = df2['z'].tolist()
fig.suptitle('Revenue Decrease Percentages across Tariff Scenarios and Elasticities',
             fontsize=100, fontweight='bold')
ax.set_xticks(X)
ax.tick_params(axis='x', which='major', pad=35)
ax.set_yticks(Y)
...
fig, ax = plt.subplots(figsize=(25, 10))
ax.set_title('Revenue Decrease Percentages across Elasticities (Tariff Scenarios: ' +
             T1+' ,'+T2+' and '+T3+')', fontdict={'fontsize': 30})
ax.set_xlabel('Elasticities', fontdict={'fontsize': 30})
...
fig, ax = plt.subplots(figsize=(25, 10))
ax.set_title('Revenue Decrease Percentages across Elasticities (Random Tariff Scenarios)',
             fontdict={'fontsize': 30})
ax.set_xlabel('Elasticities', fontdict={'fontsize': 30})
ax.set_ylabel('Revenue Decrease Percentages', fontdict={'fontsize': 30})
ax.set_xticks(df.index)
...
###
df = pd.read_excel(
    '3 Our Tariff Results/4.Net Increase Percentage.xlsx', index_col=0)
df.replace(np.nan, 0, inplace=True)
df.columns = plotting_columns
df2 = pd.DataFrame(columns=['x', 'y', 'z'])
j = 0
...
Y = df2['y'].tolist()
Z = df2['z'].tolist()
fig.suptitle('Net Increase Percentages across Tariff Scenarios and Elasticities',
             fontsize=100, fontweight='bold')
ax.set_xticks(X)
ax.tick_params(axis='x', which='major', pad=35)
ax.set_yticks(Y)
...
fig, ax = plt.subplots(figsize=(25, 10))
ax.set_title('Net Increase Percentages across Elasticities (Tariff Scenarios: ' +
             T1+' ,'+T2+' and '+T3+')', fontdict={'fontsize': 30})
ax.set_xlabel('Elasticities', fontdict={'fontsize': 30})
...
fig, ax = plt.subplots(figsize=(25, 10))
ax.set_title('Net Increase Percentages across Elasticities (Random Tariff Scenarios)',
             fontdict={'fontsize': 30})
ax.set_xlabel('Elasticities', fontdict={'fontsize': 30})
ax.set_ylabel('Net Increase Percentages', fontdict={'fontsize': 30})
ax.set_xticks(df.index)
...
###
df = pd.read_excel(
    '3 Our Tariff Results/6.Demand Shifted Percentage.xlsx', index_col=0)
df.replace(np.nan, 0, inplace=True)
df.columns = plotting_columns
df2 = pd.DataFrame(columns=['x', 'y', 'z'])
j = 0
...
Y = df2['y'].tolist()
Z = df2['z'].tolist()
fig.suptitle('Demand Shifted Percentages across Tariff Scenarios and Elasticities',
             fontsize=100, fontweight='bold')
ax.set_xticks(X)
ax.tick_params(axis='x', which='major', pad=35)
ax.set_yticks(Y)
...
fig, ax = plt.subplots(figsize=(25, 10))
ax.set_title('Demand Shifted Percentages across Elasticities (Tariff Scenarios: ' +
             T1+' ,'+T2+' and '+T3+')', fontdict={'fontsize': 30})
ax.set_xlabel('Elasticities', fontdict={'fontsize': 30})
...
fig, ax = plt.subplots(figsize=(25, 10))
ax.set_title('Demand Shifted Percentages across Elasticities (Random Tariff Scenarios)',
             fontdict={'fontsize': 30})
ax.set_xlabel('Elasticities', fontdict={'fontsize': 30})
ax.set_ylabel('Demand Shifted Percentages', fontdict={'fontsize': 30})
ax.set_xticks(df.index)
...
###
df = pd.read_excel(
    '3 Our Tariff Results/B. Response_Details.xlsx', index_col=0)

### B.1 Box_Plot_%_shifted_demand ###
df.boxplot('% of demand shifted', figsize=(15, 10), fontsize=15)
plt.title('Box plot of percentage of shifted demand across all demands (Tariff Scenario: ' +
          T+', Elasticity: '+str(e)+')', fontdict={'fontsize': 20})
...
plt.title('Box plot of amount of shifted demand across all demands (Tariff Scenario: ' +
          T+', Elasticity: '+str(e)+')', fontdict={'fontsize': 20})
...
ax1.set_title('Original Demand, Altered Demand and Energy Tariff (Tariff Scenario: ' +
              T+', Elasticity: '+str(e)+')', fontdict={'fontsize': 20})
ax1.set_xlabel('time (h)', fontdict={'fontsize': 15})
ax1.set_ylabel('Water Consumption (GPM)',
               color='red', fontdict={'fontsize': 15})
ax1.set_xticks(hours)
ax1.plot(hours, df['Original Demand'].tolist(),
         color='red', label='Original Demand (GPM)')
ax1.plot(hours, df['New Demand'].tolist(),
         color='green', label='Altered Demand (GPM)')
ax1.tick_params(axis='y', labelcolor='red')
ax1.legend(fontsize=15)
ax2 = ax1.twinx()
ax2.set_ylabel('Energy Tariff (factor/W)',
               color='blue', fontdict={'fontsize': 15})
ax2.step(hours, df['Energy Tariff'], where='post',
...
fig, ax1 = plt.subplots(figsize=(15, 10))
ax1.set_title('BAU and New Energy Cost and Energy Tariff (Tariff Scenario: ' +
              T+', Elasticity: '+str(e)+')', fontdict={'fontsize': 20})
ax1.set_xlabel('time (h)', fontdict={'fontsize': 15})
ax1.set_ylabel('Energy Cost (factor)', color='red', fontdict={'fontsize': 15})
ax1.set_xticks(hours)
ax1.plot(hours, df['Original Energy Cost'].tolist(),
         color='red', label='BAU Cost (factor)')
ax1.plot(hours, df['New Energy Cost'].tolist(),
         color='green', label='New Cost (factor)')
ax1.tick_params(axis='y', labelcolor='red')
ax1.legend(fontsize=15)
ax2 = ax1.twinx()
ax2.set_ylabel('Energy Tariff(factor/W)',
...
fig, ax1 = plt.subplots(figsize=(15, 10))
ax1.set_title('BAU and New Water Revenue and Water Tariff (Tariff Scenario: ' +
              T+', Elasticity: '+str(e)+')', fontdict={'fontsize': 20})
ax1.set_xlabel('time (h)', fontdict={'fontsize': 15})
ax1.set_ylabel('Revenue(factor)', color='red', fontdict={'fontsize': 15})
ax1.set_xticks(hours)
ax1.plot(hours, df['Original Revenue'].tolist(),
         color='red', label='BAU Revenue(factor)')
ax1.plot(hours, df['New Revenue'].tolist(),
         color='green', label='New Revenue(factor)')
ax1.tick_params(axis='y', labelcolor='red')
ax1.legend(fontsize=15)
ax2 = ax1.twinx()
ax2.set_ylabel('Water Tariff (factor/GPM)',
               color='blue', fontdict={'fontsize': 15})
ax2.step(hours, df['Water Tariff'], where='post',
         color='blue', linestyle='dotted')
ax2.tick_params(axis='y', labelcolor='blue')
fig.tight_layout()
...
fig, ax1 = plt.subplots(figsize=(15, 10))
ax1.set_title('BAU and New Water Net and Water Tariff (Tariff Scenario: ' +
              T+', Elasticity: '+str(e)+')', fontdict={'fontsize': 20})
ax1.set_xlabel('time (h)', fontdict={'fontsize': 15})
ax1.set_ylabel('Net (factor)', color='red', fontdict={'fontsize': 15})
ax1.set_xticks(hours)
ax1.plot(hours, df['Original Net'].tolist(),
         color='red', label='BAU Net (factor)')
ax1.plot(hours, df['New Net'].tolist(),
         color='green', label='New Net (factor)')
ax1.tick_params(axis='y', labelcolor='red')
ax1.legend(fontsize=15)
ax2 = ax1.twinx()
ax2.set_ylabel('Water Tariff (factor/GPM)',
...
fig, ax1 = plt.subplots(figsize=(15, 10))
ax1.set_title('Shifted Demand and Energy Differences (Tariff Scenario: ' +
              T+', Elasticity: '+str(e)+')', fontdict={'fontsize': 20})
ax1.set_xlabel('time (h)', fontdict={'fontsize': 15})
...
ax1.tick_params(axis='y', labelcolor='red')
ax2 = ax1.twinx()
ax2.set_ylabel('Energy Difference (W)', color='blue',
               fontdict={'fontsize': 15})
ax2.plot(hours, df['Energy Difference'], color='blue')
ax2.tick_params(axis='y', labelcolor='blue')
fig.tight_layout()
plt.savefig(
    '3 Our Tariff Results/D.5 Comparison_shifted_demand_and_energy_difference.png')
plt.close()

### D.6 Demand_&_Energy_original ###
fig, ax1 = plt.subplots(figsize=(15, 10))
ax1.set_title('Demand and Energy: Original (Tariff Scenario: ' +
              T+', Elasticity: '+str(e)+')', fontdict={'fontsize': 20})
ax1.set_xlabel('time (h)', fontdict={'fontsize': 15})
...
fig, ax1 = plt.subplots(figsize=(15, 10))
ax1.set_title('Demand and Energy: New (Tariff Scenario: '+T +
              ', Elasticity: '+str(e)+')', fontdict={'fontsize': 20})
ax1.set_xlabel('time (h)', fontdict={'fontsize': 15})
ax1.set_ylabel('Demand (GPM)', color='red', fontdict={'fontsize': 15})
ax1.set_xticks(hours)
...
ax1.tick_params(axis='y', labelcolor='red')
ax2 = ax1.twinx()
ax2.set_ylabel('Energy Consumption(W)', color='blue',
               fontdict={'fontsize': 15})
